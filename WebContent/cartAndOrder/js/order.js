	...
	methods:{
		...
		saveAddress:function(){
			...
			if(consumerName==""||consumerName==null||consumerName==undefined||
					...
					addressetail==null||addressetail==""||addressetail==undefined||
				...
				if(index==""||index==null||index==undefined){
					//alert(2);
					var newAddress = new createAddress(0, this.user.cid, consumerName, telephone, postcode, addressDetail, false);
...
/*			console.log("layer");
			...
			giftCardId=this.giftCards[index].giftCardId;
		},
		//计算最终应付的价格
		...
		payment:function(){
			...
			if(count==0){
				...
				var consumerId=this.user.cid;
				//alert("saveOrder");
				saveOrder(this.order,consumerId,addressId,this.totalPrice);		
			}
...
function getAddress(){
	...
	$.ajax({
		...
		data:{userId: JSON.parse(window.localStorage.getItem("user")).cid},
...
//储存订单
function saveOrder(products,consumerId,addressId,totalPrice){
	$.ajax({
		type:"post",
