$(function(){
	...
	var cname = JSON.parse(user).cname;
...
$(document).on("click", ".orderspec", function(){ 
	var orderData = JSON.parse(sessionStorage.getItem("orderData"));
   	var oid = $(this).find("span").eq(0).text();
   	var type = $(this).find("span").eq(1).text();
   	console.log(type);
   	if (type==0){
   		window.localStorage.setItem("orderId",oid);
   		window.location.href="assets/pay.html";
   		
   	}else if(type==3){
   		type=parseInt(type)+1;
   		$.ajax({   			
   			url:"updateOrderStatus/"+type+"/"+oid+".action",
   			type:"get",
   			async:false,
   			dataType:"json",
   			success:function(data){
   				window.location.href="orderDtail.html?oid="+oid;
   				},
   				error:function() {
   	   				window.location.href="orderDtail.html?oid="+oid;
   	   			}
					...
					str += '<div class="my-order" > ';
							str+='<div class = "order_top">';
								...
								str+='<div class = "row order_info">';
									str+='<div class = "col-md-8 order_info_left">'+value.startTime+' | '+cname+' | '+'订单号:'+value.orderId+' | '
									if(value.payment==null){
										str+=' </div>';
									}else{
										str+=value.payment+'</div>';
									}
							...
							str+='<div class = "row order_bottom">';
								str+='<div class = "col-md-8 order_bottom_left">';
									value.productsView.forEach(function(productview,index){
										...
										str+='<div class="col-md-3 order_bottom_left_picture"><img alt="" src="cartAndOrder/img/'+productview.picture+'/cart.jpg" ></div>'
								...
								str+='<div class = "col-md-4 order_bottom_right">'
									...
									str+='<span class="orderspec"><a class="btn btn-small btn-line-gray"   >'+action[value.orderStatus]+'</a><span style="display:none">'+value.orderId+'</span><span style="display:none">'+value.orderStatus+'</span></span>';
								str+='</div>';
							str+='</div>';
						str+='</div>';
					str+='</div>';
