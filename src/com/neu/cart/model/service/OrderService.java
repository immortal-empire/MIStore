...
public class OrderService {
	...
	@Autowired
	private ShoppingCartInfoDAO shoppingCardDAO;
	...
	public int saveOrder(Order order) {
		...
		if(order.getGiftCard()!=null){
			...
			giftmap.put("Id", order.getGiftCard().getGiftCardId());
			giftmap.put("balance", order.getGiftCard().getBalance());
			if(order.getGiftCard().getBalance()==0) {
				giftmap.put("status", "0");
			}else {
				giftmap.put("status", "1");
			}
			shoppingCardDAO.updateGiftCard(giftmap);
		}
	...
	/*
	 * 添加评论时须将orderItem中ifevaluate属性置为'1'
	 * */
	public void setIfEvaluate(int orderId, int productId) {
		orderDAO.setIfEvaluate(orderId, productId);
	}
}
